...
<div id="content">
    <form method="get" action="/surfaces/${surface.id}/findPages">
        ...
        <p><input name="word" type="text" placeholder="Введите название страницы"/></p>
    ...
    <p>${surface.description}</p>
        ...
        <c:otherwise>
            <p>У этого пространтсва отсутствует описание</p>
        </c:otherwise>
    </c:choose>
    <h2>Пользователи:</h2>
    ...
    <div class="dropdown" id="settings">
        ...
        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
            <c:if test="${user.id == surface.adminId}">
            <a class="dropdown-item" href="/surfaces/delete/${surface.id}" onclick="return del()">Удалить</a>
            </c:if>
            <c:if test="${surface.adminId == user.id || surface.accessType == 'private'}">
            <a class="dropdown-item" href="/surfaces/${surface.id}/pages/new">Добавить корневую страницу</a>
            </c:if>
            <a class="dropdown-item" href="/surfaces/${surface.id}/findPages">Поиск страниц</a>
            <c:if test="${user.id == surface.adminId}">
            <a class="dropdown-item" href="/surfaces/edit/${surface.id}">Редактировать</a>
            </c:if>
            <c:if test="${user.id == surface.adminId}">
                <a class="dropdown-item" href="/surfaces/${surface.id}/findUser">Добавить нового пользователя</a>
            </c:if>
            <c:if test="${surface.accessType == 'public' && !surface.users.contains(user)}">
                <a class="dropdown-item" href="/surfaces/${surface.id}/addUser/${user.id}">Присоединиться</a>
            </c:if>
        </div>
    </div>
</div>
